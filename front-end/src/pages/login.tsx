import React, { useReducer, useEffect, MouseEvent } from 'react';
...
const LoginPage = () => {
  ...
  async function handleLogin(event: MouseEvent<HTMLButtonElement>)
    ...
    let payload: LoginApiPayload = {
		...
		// try
		// {
		// 	const response: Response = await fetch(buildpath("/api/login"),
		// 		{
		// 			method: "POST",
		...
		// 	let responseJson: UserDataWithoutPassword = await JSON.parse(await response.());
		// }
		// catch (e)
		// {
		// 	alert(e.toString());
  ...
  return (
    <form className={classes.container+' abc'} noValidate autoComplete="off">
      <Card className={classes.card}>
	  	<Image src={process.env.PUBLIC_URL + "/Bitcoin_logo.svg"} fluid />
			<CardContent>
						<div>
							<TextField
								error={state.isError}
